    ...
    while R_flag == 0: # 빨간색 미션 안끝났을 때
        ...
        if Dec_r == 0: # 빨간색 도형을 못찾으면
            ...
            if right_flag == 0 and left_flag == 0 and top_flag == 0 and bottom_flag == 0:
                #도형의 중심이 중앙에 있을 때
                # tello.move_forward(40) #당일 조정가능, 앞으로 이동(크기가 작은 것을 고려)
                print("foward 40")
                print(type)
                time.sleep(1)
                if type == 1: # Front이면
...
while True:
    ...
    img_hsv = cv.cvtColor(img_color, cv.COLOR_BGR2HSV) #웹캠은 BGR로 드론은 RGB로
    ...
    if qr == 0: #qr 판독 안했을 때
        if find_r == 1: # 빨간색 탐색했을 때
            if objectType != "": # 도형이 있다면
                    Dec_r = 1
